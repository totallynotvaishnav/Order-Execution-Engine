...
export class TransactionRegistry {
  ...
  async modifyTransaction(transactionId: string, updates: Partial<Transaction>): Promise<Transaction> {
  ...
  async markAsCompleted(transactionId: string, txHash: string, executedPrice: number): Promise<Transaction> {
  ...
  async markAsFailed(transactionId: string, errorMessage: string, retryCount: number): Promise<Transaction> {
    return await this.modifyTransaction(transactionId, {
      status: "failed",
      errorMessage,
      retryCount,
  ...
  async listPendingTransactions(): Promise<Transaction[]> {
    const pendingStates: TransactionState[] = [
      "pending",
      "routing",
      "building",
      "submitted",
    ...
    return allTransactions.filter((transaction) =>
      pendingStates.includes(transaction.status)
    );
  }
